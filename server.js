  ...
  app.get("/admin-menu.html", async (req, res) => {
    if (
      ...
      let html = fs.readFileSync(
        path.join(__dirname, "admin-menu.html"),
        "utf8",
      );
    ...
    try {
      ...
      const notesWithUrl = notes.map((note) => {
        ...
        return {
          ...
          formattedDate: new Date(note.uploaded_at).toLocaleDateString(
            "en-US",
            {
              year: "numeric",
              month: "short",
  ...
  app.post("/submit-answer", async (req, res) => {
    ...
    try {
      ...
      const [existing] = await db.execute(
        "SELECT * FROM q_bank_answers WHERE email = ? AND question_id = ?",
        [req.session.user, questionId],
      );
      if (existing.length > 0) {
        return res.json({ success: false, message: "Already answered" });
      }
      await db.execute(
        "INSERT INTO q_bank_answers (email, question_id, user_answer) VALUES (?, ?, ?)",
        [req.session.user, questionId, userAnswer],
      );
      // Check if correct
      ...
      if (!isCorrect) {
        await db.execute(
          ...
          [
            req.session.user,
            q.question,
            JSON.stringify([q.opt1, q.opt2, q.opt3, q.opt4]),
            q.correct,
            userAnswer,
          ],
        );
